  ...
  return (
    ...
    >
      ...
      {numSelected == 1 ? (
        ...
        <Tooltip title="Schüler auswählen">
          <IconButton>
            <ArticleIcon />
          </IconButton>
        </Tooltip>
...
export default function EnhancedTable() {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof Data>('thema');
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelecteds = rows.map((n) => n.schueler);
          ...
          >
            ...
            <TableBody>
              ...
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  const isItemSelected = isSelected(row.schueler);
                  const labelId = `enhanced-table-checkbox-${index}`;

                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, row.schueler)}
                      role="checkbox"
